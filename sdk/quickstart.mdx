...
<Steps>
  ...
  <Step title="Place a limit order">
    ...
    const result = await hip4.trading.placeOrder({
      ...
      type: "limit",
      price: String(markPx),
      amount: String(getMinShares(markPx)), // minimum shares for 1 USDC notional
    ...
    `getMinShares(markPx)` computes the minimum number of shares required to meet the 1 USDC notional floor at the given price. Passing `markPx` to `placeOrder` enables the SDK to validate this before signing and submitting.
  </Step>
</Steps>
