...
<Tabs>
  ...
  <Tab title="labelledBinary">
    Standalone binary markets with custom side names. These are one-off markets not grouped under a parent question. The sides can be anything - "Hypurr" vs "Usain Bolt", "Pass" vs "Fail" - as defined by the market creator.

    **Distinguishing fields:** custom `sides[n].name` values (not "Yes"/"No")

    ```typescript
    import type { LabelledBinaryMarket } from "@outcomexyz/hip4-sdk";
  ...
  <Tab title="multiOutcome">
    ...
    **Distinguishing fields:** `questionId`, `questionName`, `questionDescription`, `isFallback`
    ...
    ```typescript
    import type { MultiOutcomeMarket } from "@outcomexyz/hip4-sdk";
  ...
  <Tab title="priceBucket">
    Multi-bucket price range markets (introduced in v2). Instead of a single binary threshold, the parent question defines N price thresholds that create N+1 buckets. Each bucket is its own `priceBucket` market, representing whether the price will land in that range at expiry.
    ...
    **Distinguishing fields:** `priceThresholds`, `lowerBound`, `upperBound`, `bucketIndex`, `isFallback`
    ...
    ```typescript
    import type { PriceBucketMarket } from "@outcomexyz/hip4-sdk";
...
```typescript
import type { PredictionEvent, PredictionMarket, PredictionOutcome } from "@outcomexyz/hip4-sdk";
