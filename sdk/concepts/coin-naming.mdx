...
<Note>
  Outcome is migrating to native USDC settlement, but mainnet still defaults to USDH per outcome (each outcome carries its own `quoteToken`, exposed as `market.raw.quoteToken`). The USDH collateral token and `buyUsdh` / `sellUsdh` described below remain in use during the transition.
</Note>
