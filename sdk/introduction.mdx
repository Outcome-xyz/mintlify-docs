...
`@hip4/sdk` — a modular stack of HIP-4 & its financial primitives.

<div className="hero-frame">
  <img
    src="/images/sdk-architecture.svg"
    alt="HIP-4 SDK stack: outcome.xyz consumes the SDK, which composes six sub-adapters atop a single HIP-4 client"
  />
</div>
...
<CardGroup cols={2}>
  <Card title="Market discovery" icon="search" href="/sdk/guides/fetch-markets">
    Fetch and filter prediction markets by type (`defaultBinary`, `labelledBinary`, `multiOutcome`, `priceBucket`), category, or status. Paginate results and group by type or parent question.
  </Card>
