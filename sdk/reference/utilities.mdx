...
Returns the minimum number of shares required to meet a minimum notional, calculated as `ceil(minNotional / min(markPx, 1 - markPx))` - using the cheaper of the two binary-outcome sides, since that side needs more shares to reach the same notional value. `minNotional` is optional and defaults to `MIN_NOTIONAL` ($1) - pass it explicitly to compute the minimum shares against a different floor, such as an adapter's configured `minOrderNotional`.

```typescript
import { getMinShares } from "@outcome.xyz/hip4";

getMinShares(0.65);    // 3
getMinShares(0.10);    // 10
getMinShares(0.65, 10); // 29 - minimum shares for a $10 floor instead of the $1 default
```

### `MIN_NOTIONAL`

The minimum order size in USDC. Currently `1`. This is the real, unchangeable Hyperliquid protocol minimum - orders below this threshold are rejected before submission.
...
Estimates the USDC cost for a given token amount. For limit orders, uses `limitPriceCents`. For market orders, uses `marketPriceCents` when provided, otherwise returns `tokenAmount` as a fallback.
