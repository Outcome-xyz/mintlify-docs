...
Returns the minimum number of shares required to meet a minimum notional for a given mark price. `minNotional` is optional and defaults to `MIN_NOTIONAL` ($1) - pass it explicitly to compute the minimum shares against a different floor, such as an adapter's configured `minOrderNotional`.
