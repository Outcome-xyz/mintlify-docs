...
<Note>
  Outcome is migrating to native USDC settlement. `buyUsdh` and `sellUsdh` are still used today for USDH-denominated balances (mainnet still defaults to USDH) and are phased out per-outcome as USDC rolls out.
</Note>
