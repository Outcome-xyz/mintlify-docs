---
title: "Deposit, withdraw, and convert USDC on HIP-4 wallets"
sidebarTitle: "Wallet"
description: "Deposit USDC, buy and sell USDC, move funds between spot and perps, and withdraw to external addresses using the HIP-4 wallet adapter."
---
...
// Step 2: Buy USDC on the spot market (L1 agent signing)
const buy = await hip4.wallet.buyUsdc("100");
if (!buy.success) {
  throw new Error(`Buy failed: ${buy.error}`);
}

console.log(`Bought ${buy.filledSz} USDC at avg price ${buy.avgPx}`);
```
...
```typescript
// Step 1: Sell USDC on the spot market (L1 agent signing)
const sell = await hip4.wallet.sellUsdc("50");
if (!sell.success) {
