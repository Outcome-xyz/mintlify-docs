...
individual `placeOrder` call to bypass this floor - and the protocol one - entirely. See
...
<ParamField path="markPx" type="number">
  Current market price (0–1). When provided, the SDK enforces a minimum shares check: `amount >= getMinShares(markPx)`, which ensures the order meets the [configured minimum notional](#minordernotional) (1 USDC by default).
</ParamField>
