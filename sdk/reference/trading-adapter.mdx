...
- Validates that the notional value (`price × amount`) meets the minimum (10 USDC) unless `skipMinNotionalCheck` is `true`.
...
<ParamField path="markPx" type="number">
  Current market price (0–1). When provided, the SDK enforces a minimum shares check: `amount >= getMinShares(markPx)`, which ensures the order meets the 10 USDC minimum notional.
</ParamField>
...
<ParamField path="skipMinNotionalCheck" type="boolean">
  ...
  below 10 USDC but the exchange still accepts the order.
</ParamField>
