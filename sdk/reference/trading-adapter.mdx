...
<ParamField path="skipMinNotionalCheck" type="boolean">
  ...
  below 10 USDH but the exchange still accepts the order.
</ParamField>
...
<ParamField path="amount" type="string" required>
  USDH amount to split, as a decimal string (e.g. `"12.5"`). The SDK strips
  trailing zeros to match Hyperliquid's wire format. `X` USDH burned → `X` Yes +
  `X` No minted.
</ParamField>
