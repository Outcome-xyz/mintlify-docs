...
The HIP-4 SDK exposes four share-conversion methods on the trading adapter. They settle directly against your spot balances at the protocol mint price - no orderbook, no slippage, no spread paid. See the [Converting tokens concept page](/platform/trading/converting) for the underlying bundle equivalences.
...
<Warning>
  Conversions affect your spot balances directly. There is no "are you sure?"
  step - a successful call mints or burns shares immediately. Validate inputs
  before submitting, especially `amount` strings.
</Warning>
...
const result = await hip4.trading.splitOutcome({
  ...
  amount: "100", // 100 USDC burned, 100 Yes + 100 No minted
});
...
const result = await hip4.trading.mergeOutcome({
  outcome: 5160,
  amount: "100", // 100 paired shares burned, 100 USDC minted
});
```
...
const result = await hip4.trading.mergeQuestion({
  ...
  amount: "10", // 10 USDC minted; 10 Yes burned from each outcome
});
```
...
<Note>
  The on-wire sub-key is `negateOutcome` (matching Hyperliquid's testnet
  "Convert Outcomes" UI). Hyperliquid's docs body shows `negateQuestion` in
  places - that's a typo. The SDK uses the correct key.
</Note>
...
if (!result.success) {
  ...
  // "Insufficient USDC balance" - not enough collateral to split
...
if (redeem.success) {
  console.log("Redeemed full Yes-bundle for USDC");
}
```
...
- [Converting tokens concept](/platform/trading/converting) - the bundle equivalences behind these primitives
