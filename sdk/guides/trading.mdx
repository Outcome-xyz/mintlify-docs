...
<Steps>
  ...
  <Step title="Fetch the market and side">
    ...
    ```typescript
    import type { DefaultBinaryMarket } from "@outcome.xyz/hip4outcome.xyz/hip4";
  ...
  <Step title="Place the limit order">
    ...
    ```typescript
    import { getMinShares } from "@outcome.xyz/hip4outcome.xyz/hip4";
...
**Min-notional** - orders below 1 USDC notional value are rejected client-side before the order is signed or sent. Notional is calculated as `price × amount`. Pass `markPx` to enable this check; omit it to skip it. Need a stricter floor than the protocol minimum? Set `minOrderNotional` on `createHIP4Adapter` - see [`minOrderNotional`](/sdk/reference/trading-adapter#minordernotional) in the Trading Adapter reference.
