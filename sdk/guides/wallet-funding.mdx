...
<Note>
  Outcome is migrating to native USDC settlement. On mainnet, USDH is still the default quote token, so the `buyUsdh` / `sellUsdh` spot steps below remain in use during the transition and are phased out per-outcome as USDC rolls out.
</Note>
