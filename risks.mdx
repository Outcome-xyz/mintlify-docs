...
Deposits from other chains are handled by third parties. USDC from supported networks moves through [Circle's CCTP](https://www.circle.com/cross-chain-transfer-protocol); other tokens, and deposits sent to a deposit address, go through [Across](https://across.to/), a cross-chain bridge. Both have strong track records, but bridges remain a higher-risk category in DeFi. A failure would affect deposits in transit but would not affect funds already in your account on Hyperliquid.

Cash deposits are handled by [Swapped.com](https://swapped.com/), which processes the payment and holds any identity documents you submit. Their availability, limits, and data handling are theirs, not Outcome's.

Outcome is not affiliated with any of these providers and cannot reverse a transfer once it has left your wallet. See [Funding your account](/funding-your-account) for which route applies to each deposit method.
