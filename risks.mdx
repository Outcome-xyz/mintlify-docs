...
Markets resolve based on the resolution terms defined in the market specification. Price-based markets settle against a TWAP of the underlying taken around expiry. Event markets (sports, macro data, and similar) are resolved against authoritative sources through Outcome's resolution system. In both paths, settlement depends on the integrity of the resolution process: a compromised, manipulated, or stale price feed, or an incorrect event resolution, could cause settlement to behave unexpectedly. For event markets specifically, outcomes that are ambiguous as worded carry more resolution uncertainty than deterministic price readings.
