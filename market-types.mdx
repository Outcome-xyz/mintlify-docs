...
Validator-deployed binary crypto markets resolve on HyperCore data. The resolution is performed by the Hyperliquid validator set itself, not by Outcome.

Mechanically they trade like every other market on Outcome: same order book, same outcome tokens, same settlement to 1 or 0 USDC.
