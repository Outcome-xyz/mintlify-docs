...
The minimum order size is 1 USDC in value. For a buy at 50 cents per token, that's at least 2 tokens. For a buy at 10 cents per token, at least 10 tokens. The minimum scales with the price so the total value stays at least 1 USDC.
